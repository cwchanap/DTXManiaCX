...
The implementation should therefore reuse the current system-key pipeline end to end. Add `OpenSearch` to the panel's existing action list, keep it optional and unbound by default, and add focused regression coverage proving the existing SQLite persistence and Song Select dispatch paths work with the newly reachable command.
...
This means the production behavior change should be limited to the System Key Mapping panel unless a focused test proves an existing generic seam is broken.
...
The existing 540px panel still fits the ninth action row plus Save/Cancel footer, so no layout change is required.
...
The raw Backspace shortcut stays unchanged. Backspace is also the key-capture cancel key in `SystemKeyAssignPanel`; HPA-23 does not try to normalize that special case into the remappable command path.

`Tab` also remains the existing raw Song Select tab-switch shortcut. HPA-23 does not refactor raw stage shortcuts or create a reserved-key framework; focused acceptance uses an ordinary non-reserved key such as `F1`.
...
Add only focused tests around the newly reachable path:

1. **Panel exposure** — prove `SystemKeyAssignPanel.Actions` contains `OpenSearch`; the existing formatter test already pins the `Open Search` label.
2. **Persistence round trip** — save a non-reserved key such as `F1` for `OpenSearch`, flush the SQLite-backed config, reload it, and prove a fresh configured `InputManager` restores `F1 -> OpenSearch`.
3. **Song Select command suppression** — enqueue `OpenSearch` followed by a normal song-list command, process the queue, and prove the modal opens while the following command does not move the selection.
4. Keep the existing Backspace tests unchanged as regression coverage for the legacy shortcut.
...
- System Key Mapping shows `Open Search`.
