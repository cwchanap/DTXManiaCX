# HPA-519 Managed Crash Reporting — Design

**Date:** 2026-08-02  
**Status:** Approved  
**Linear:** HPA-519 — Crash report  
**Scope:** Capture managed fatal crashes locally, retain a small sanitized diagnostic bundle, and offer a non-blocking GitHub issue handoff on the next launch.

## Summary

DTXManiaCX will add a dedicated crash-reporting subsystem that is available before MonoGame initialization and remains alive for the full process lifetime. When a managed exception escapes the game startup or `Game.Run()` loop, the subsystem writes a sanitized local crash bundle. On the next successful visit to the title screen, the player sees a non-blocking notification and may review the report, open a prefilled GitHub issue, open the report folder, dismiss the notification, or delete the report.
...
| Failure scope | Managed fatal crashes only |
...
- Capture exceptions thrown during game construction, startup, or the MonoGame run loop when they reach the process boundary.
- Preserve useful evidence even when graphics, audio, input, configuration, or SQLite initialization is incomplete.
- Include recent structured logs, semantic breadcrumbs, safe runtime context, and an exception chain.
...
- Keep crash reporting isolated, testable, and reusable for later native-dump or hang-report extensions.
...
- Broad `AppDomain.UnhandledException` or unobserved-task handlers that may duplicate the process-boundary report.
...
Create a `CrashReportRuntime` before constructing `Game1`:
...
Only exceptions reaching the top-level process boundary produce a report. Recoverable exceptions that are caught and logged by game code do not create crash bundles.
...
| `CrashContextCollector` | Collect independent snapshots from registered providers |
| `ICrashContextProvider` | Return one bounded immutable context section |
| `CrashBreadcrumbBuffer` | Retain recent semantic lifecycle and device events |
| `CrashLogBufferProvider` | Retain recent structured `ILogger` entries in memory |
...
No static service locator is introduced. `TitleStage` cannot create or inspect ZIP internals and does not know the storage format. It receives only the inbox operations needed for presentation.
...
2. Capture the exception type, message, stack trace, and inner-exception chain.
3. Invoke each registered context provider independently.
...
The capture path is synchronous, bounded, dependency-light, and best effort. It does not require the network, SQLite, graphics, audio, or the MonoGame update loop.
...
- application version and build/commit identifier when available;
...
- context-provider status and collection errors;
...
- sanitized rendered message;
- sanitized exception summary when attached.
...
- Context providers have fixed output limits.
...
- absolute song, chart, skin, cache, configuration, and temporary paths;
...
- whether the Game API is enabled, never its port-facing key;
- selected input mode;
...
If a field cannot be safely sanitized, persist `[REDACTED]`. A sanitizer error must never cause the original unsanitized value to be written.
...
The crash buffer is not a permanent rolling log. It exists solely to preserve recent evidence if the process terminates unexpectedly.
...
Stage transitions should be emitted centrally by `StageManager`. Device events should be emitted by the subsystem that owns the device state.
...
- has a strict execution and output-size bound;
...
When several reports exist, Previous and Next navigate the retained reports. The panel may consume title-screen input while open. `Escape` or Dismiss returns to the normal title menu immediately.
...
Open the DTXManiaCX new-issue page with a prefilled title and body containing only:
...
- Report deletion from the UI requires one deliberate action; no automatic confirmation dialog is required unless consistent existing UI patterns make accidental activation likely.
...
- redaction of home paths, app-data paths, API keys, usernames, song names, and user filenames;
...
- acknowledgement, deletion, and retention-state cleanup;
...
- Delete removes the selected report and refreshes counts;
- failed browser or folder launch displays an error and does not acknowledge.

### Integration verification

Use `DTXMANIA_APPDATA_ROOT` to isolate test output:

1. Inject a synthetic exception before graphics initialization and verify a report is produced.
2. Inject a synthetic exception after reaching the title stage and verify stage context and recent logs are present.
...
5. Inspect all bundle entries for prohibited path, key, username, song, and filename data.
...
- Bounded structured log and breadcrumb buffers.
- Context-provider registration and independent collection.
...
- A synthetic exception during the game loop includes available context, logs, and breadcrumbs.
- One failed provider does not prevent report creation.
- No secret, user path, username, song identity, or arbitrary configuration value is persisted.
...
- Previous/Next navigation, acknowledgement, dismissal, and deletion.
...
- Deleting a report removes the file and updates notification state.
...
1. A managed fatal exception produces a completed sanitized local report.
