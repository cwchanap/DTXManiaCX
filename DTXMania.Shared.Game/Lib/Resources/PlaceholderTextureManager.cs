...
namespace DTX.Resources
{
    ...
    public class PlaceholderTextureManager : IDisposable
    {
        ...
        {
            if (_placeholderCache.TryGetValue(key, out var cachedTexture) && cachedTexture != null)
            {
                try
                {
                ...
                {
                    // Remove disposed texture from cache and create new one
                    _placeholderCache.TryRemove(key, out _);
                }
            }
